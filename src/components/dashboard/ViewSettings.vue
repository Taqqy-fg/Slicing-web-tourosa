...
defineProps({
  ...
  tabCatBg: String,
  tabCatColor: String,
  isTabWebsite: Boolean,
  isTabCatalog: Boolean,
...
<template>
  <div style="padding:30px 32px;max-width:1000px;margin:0 auto;">
    <div style="display:inline-flex;background:#fff;border:1px solid #e8e9ee;border-radius:12px;padding:5px;margin-bottom:22px;gap:4px;">
    ...
    <div v-if="isTabWebsite" style="display:flex;flex-direction:column;gap:18px;">
      ...
      <div style="background:#fff;border:1px solid #e8e9ee;border-radius:16px;padding:24px;">
        ...
        <div v-for="(cl, idx) in clientsEdit" :key="idx" style="display:grid;grid-template-columns:130px 1fr 36px;gap:12px;align-items:center;padding:9px 0;border-top:1px solid #f1f2f5;">
          ...
          <input :value="cl.name" @input="cl.onName" placeholder="Nama klien" style="width:100%;padding:11px 13px;border:1px solid #d8dce4;border-radius:9px;font-size:14px;color:#1a2235;background:#fff;outline:none;">
